...
func (e *Engine) productionResultState(
	...
	if record.Decision == OutputQuarantine || record.Decision == OutputDeny {
		denyResultForOutputPolicy(result, record.Reason)
...
// denyResultForOutputPolicy corrects a JobResult in place to reflect a
// scheduler-side output-policy quarantine/deny decision. Both the compat
// (handleJobResult, engine.go) and production/durable (productionResultState,
// this file) paths compute the terminal disposition into a local model.JobState
// without this correction the worker's original JOB_STATUS_SUCCEEDED status
// would still be republished verbatim onto SubjectAcceptedResult/
// SubjectAcceptedProgress (engine_accepted_event.go), and the workflow
// engine's applyResult() (core/workflow/engine_state.go) would record the
// step as succeeded using the quarantined/unsafe result pointer — an
// output-policy bypass at the workflow layer.
func denyResultForOutputPolicy(result *pb.JobResult, reason string) {
	if result == nil {
		return
	}
	...
	result.Status = pb.JobStatus_JOB_STATUS_DENIED
	result.ErrorCode = outputPolicyReason
	result.ErrorCodeEnum = pb.ErrorCode_ERROR_CODE_SAFETY_DENIED
