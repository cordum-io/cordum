...
| Safety Kernel or policy evaluate appears unavailable. | Gateway cannot reach Safety Kernel or policy route returns 5xx. | Run doctor; check Gateway logs and `SAFETY_KERNEL_ADDR`; in `observe` mode evidence may be degraded; `enterprise-strict` fails risky actions closed. `enforce` fails risky actions closed **only when `CORDUM_AGENTD_FAIL_CLOSED=true`**, which is not the default; without it an `enforce` session fails **open** on agentd error/timeout. Do not relabel degraded evidence as allow. |
