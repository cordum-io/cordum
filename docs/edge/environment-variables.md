...
| `CORDUM_EDGE_LLM_INGEST_ENABLED` | unset (treated as disabled) | bool — `true` / `1` / `yes` enable (case-insensitive); anything else disables | Set to `true` to expose `POST /api/v1/edge/llm/events` (LLM-proxy chat-turn redaction + audit). When unset the route returns `503 service_unavailable` with no Redis writes. | [`llm-proxy-governance.md` §disabled-by-default](llm-proxy-governance.md), `core/controlplane/gateway/handlers_edge_llm_ingest.go` |
| `CORDUM_EDGE_LLM_REPLAY_REQUIRED` | unset (treated as optional) | bool-ish opt-in — only `true` / `1` / `yes` make the nonce mandatory | Unlike runtime telemetry, an LLM proxy issues one synchronous call per chat turn, so a replay `nonce` is optional by default (deduplicated when present). Set `true` to mandate it. | [`llm-proxy-governance.md` §wire envelope](llm-proxy-governance.md), `core/edge/llmingest/adapter.go` |
