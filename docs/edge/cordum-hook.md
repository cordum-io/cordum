...
| `enterprise-strict` or `CORDUM_AGENTD_FAIL_CLOSED=true` | Fail closed. For parseable `PreToolUse`, emit structured deny JSON; for hooks without a safe block JSON, exit `2`. | Exit `2`, empty stdout. |
...
For Claude Code hook JSON semantics, see the official Claude Code hooks documentation: <https://docs.anthropic.com/en/docs/claude-code/hooks>.
