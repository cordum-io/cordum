...
| Worker readiness handshake | Yes | Unit (pinned CAP SDK + scheduler filter) | `Agent.Start()` auto-publishes a `Handshake{ready_topics}` payload inside a `BusPacket` whose envelope carries the session `auth_token` (`BusPacket.auth_token`, not a `Handshake` field); scheduler filters on `ready=true` when `WORKER_READINESS_REQUIRED=true`. |
