...
The dashboard implements the **Control Surface** design language — a dark-first,
compact, data-dense interface for infrastructure operators. All colors, surfaces,
and typography are driven by CSS custom properties so themes can be swapped
without modifying component code.

### Parity Validation

After any styling, theming, or component changes, run the full validation:
...
Tests in `src/styles/` verify that CSS variable tokens, Tailwind mappings, and
component classes stay aligned with the design spec. These run as part of the
normal test suite and will fail if a token is removed or renamed without updating
all consumers.

### Reference Documents

- [`DESIGN_PARITY_CHECKLIST.md`](./DESIGN_PARITY_CHECKLIST.md) — token-by-token parity evidence
- [`DESIGN_LANGUAGE_MAPPING.md`](./DESIGN_LANGUAGE_MAPPING.md) — spec-to-code mapping table
- [Wiki: Dashboard Design System](../wiki/Dashboard.md) — overview and cross-platform notes
