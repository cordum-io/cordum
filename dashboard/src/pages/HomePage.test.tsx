...
describe("HomePage (Security Overview)", () => {
  ...
  it("renders KPI metric cards when loaded", () => {
    mockStatus.data = {
      nats: { connected: true },
      redis: { ok: true },
      uptime_seconds: 3600,
  ...
  it("shows worker count from workers data", () => {
  ...
  it("shows NATS Connected when nats is connected", () => {
    mockStatus.data = { nats: { connected: true }, redis: { ok: true }, uptime_seconds: 0, workers: { count: 0 } };
    render();
  ...
  it("shows NATS Disconnected when nats is not connected", () => {
