...
const NODE_TYPES: { type: NodeType; label: string; icon: LucideIcon; color: string; desc: string }[] = [
...
export default function WorkflowBuilderPage() {
  ...
  useEffect(() => {
    ...
    const loadedNodes: BuilderNode[] = (existingWorkflow.steps ?? []).map((s, i) => {
      nodeCounterRef.current = Math.max(nodeCounterRef.current, i + 1);
      const nodeType: NodeType = s.type === "job" ? "worker" : s.type === "sub-workflow" ? "subworkflow" : (s.type as NodeType);
      return {
        id: s.id ?? `node-${i + 1}`,
        type: nodeType,
        ...
        config: (s.config ?? {}) as Record<string, string | number | boolean>,
      };
    });
    setNodes(loadedNodes);
    const loadedEdges: BuilderEdge[] = [];
    for (const s of existingWorkflow.steps ?? []) {
  ...
  const handleDeploy = () => {
    ...
    const steps: WorkflowStep[] = nodes.map(node => ({
      id: node.id,
      name: node.label,
      type: node.type === "worker" ? "job" : node.type === "subworkflow" ? "sub-workflow" : node.type,
