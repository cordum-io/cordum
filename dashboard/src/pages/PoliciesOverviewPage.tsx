...
import { computeGovernanceScore, getScoreColor } from "../lib/governanceScore";
...
function GovernanceScoreCard({
  ...
  const variant = score > 80 ? "default" : score >= 50 ? "warning" : "danger";
        ...
        <div className="relative h-28 w-28 shrink-0">
          ...
          <span
            className={cn(
              ...
              {
                "text-success": getScoreColor(score) === "success",
                "text-warning": getScoreColor(score) === "warning",
                "text-danger": getScoreColor(score) === "danger",
              }
            )}
          >
            {score}
...
function DecisionDistributionChart({
  ...
  const data = useMemo(() => {
    ...
    return Object.entries(counts).map(([name, value]) => ({
      ...
      value,
      color: DECISION_COLORS[name] ?? "var(--muted-foreground)",
    }));
  }, [auditEntries]);
