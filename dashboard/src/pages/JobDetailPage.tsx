...
function JobTimeline({ job }: { job: Job }) {
  ...
  return (
    <div className="relative pl-6 space-y-4">
      ...
      {entries.map((entry) => (
        <div key={entry.label} className="relative flex items-start gap-3">
          <div
            className={cn(
                  ...
                  {job.output_safety.findings && job.output_safety.findings.length > 0 && (
                    <div className="mt-3 space-y-2">
                      ...
                      {job.output_safety.findings.map((f: OutputFinding) => (
                        <div key={`${f.type}-${f.scanner ?? ""}-${f.detail.slice(0, 40)}`} className="rounded-md bg-surface-0 border border-border p-3">
