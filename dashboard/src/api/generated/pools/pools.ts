/**
 * Generated by orval 🍺
 * Do not edit manually.
 * Cordum HTTP API
 * Canonical OpenAPI 3.0.3 spec for the Cordum gateway HTTP surface.
 * OpenAPI spec version: 2026-05-09.2
 */
import { useMutation, useQuery } from "@tanstack/react-query";
import type {
  DataTag,
  DefinedInitialDataOptions,
  DefinedUseQueryResult,
  MutationFunction,
  QueryClient,
  QueryFunction,
  QueryKey,
  UndefinedInitialDataOptions,
  UseMutationOptions,
  UseMutationResult,
  UseQueryOptions,
  UseQueryResult,
} from "@tanstack/react-query";

import type {
  BadRequestResponse,
  ConflictResponse,
  CreatePoolBody,
  DeletePoolParams,
  DrainPoolBody,
  ForbiddenResponse,
  ListPools200,
  NotFoundResponse,
  PoolDetail,
  PoolMutation,
  UnauthorizedResponse,
  UpdatePoolBody,
} from "../model";

import { apiClient } from "../../client.ts";

const withQueryKey = <T extends object, K>(
  query: T,
  queryKey: K,
): T & { queryKey: K } => {
  const result = { queryKey } as T & { queryKey: K };
  for (const key of Object.keys(query)) {
    // The explicit queryKey always wins, matching the previous
    // `{ ...query, queryKey }` spread where it was set last.
    if (key === "queryKey") continue;
    Object.defineProperty(result, key, {
      enumerable: true,
      configurable: true,
      get: () => (query as Record<string, unknown>)[key],
    });
  }
  return result;
};

/**
 * @summary List worker pools
 */
export const listPools = (signal?: AbortSignal) => {
  return apiClient<ListPools200>({
    url: `/api/v1/pools`,
    method: "GET",
    signal,
  });
};

export const getListPoolsQueryKey = () => {
  return [`/api/v1/pools`] as const;
};

export const getListPoolsQueryOptions = <
  TData = Awaited<ReturnType<typeof listPools>>,
  TError = UnauthorizedResponse | ForbiddenResponse,
>(options?: {
  query?: Partial<
    UseQueryOptions<Awaited<ReturnType<typeof listPools>>, TError, TData>
  >;
}) => {
  const { query: queryOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getListPoolsQueryKey();

  const queryFn: QueryFunction<Awaited<ReturnType<typeof listPools>>> = ({
    signal,
  }) => listPools(signal);

  return { queryKey, queryFn, ...queryOptions } as UseQueryOptions<
    Awaited<ReturnType<typeof listPools>>,
    TError,
    TData
  > & { queryKey: DataTag<QueryKey, TData, TError> };
};

export type ListPoolsQueryResult = NonNullable<
  Awaited<ReturnType<typeof listPools>>
>;
export type ListPoolsQueryError = UnauthorizedResponse | ForbiddenResponse;

export function useListPools<
  TData = Awaited<ReturnType<typeof listPools>>,
  TError = UnauthorizedResponse | ForbiddenResponse,
>(
  options: {
    query: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof listPools>>, TError, TData>
    > &
      Pick<
        DefinedInitialDataOptions<
          Awaited<ReturnType<typeof listPools>>,
          TError,
          Awaited<ReturnType<typeof listPools>>
        >,
        "initialData"
      >;
  },
  queryClient?: QueryClient,
): DefinedUseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useListPools<
  TData = Awaited<ReturnType<typeof listPools>>,
  TError = UnauthorizedResponse | ForbiddenResponse,
>(
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof listPools>>, TError, TData>
    > &
      Pick<
        UndefinedInitialDataOptions<
          Awaited<ReturnType<typeof listPools>>,
          TError,
          Awaited<ReturnType<typeof listPools>>
        >,
        "initialData"
      >;
  },
  queryClient?: QueryClient,
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useListPools<
  TData = Awaited<ReturnType<typeof listPools>>,
  TError = UnauthorizedResponse | ForbiddenResponse,
>(
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof listPools>>, TError, TData>
    >;
  },
  queryClient?: QueryClient,
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
/**
 * @summary List worker pools
 */

export function useListPools<
  TData = Awaited<ReturnType<typeof listPools>>,
  TError = UnauthorizedResponse | ForbiddenResponse,
>(
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof listPools>>, TError, TData>
    >;
  },
  queryClient?: QueryClient,
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
} {
  const queryOptions = getListPoolsQueryOptions(options);

  const query = useQuery(queryOptions, queryClient) as UseQueryResult<
    TData,
    TError
  > & { queryKey: DataTag<QueryKey, TData, TError> };

  return withQueryKey(query, queryOptions.queryKey);
}

/**
 * @summary Get pool detail
 */
export const getPool = (name: string, signal?: AbortSignal) => {
  return apiClient<PoolDetail>({
    url: `/api/v1/pools/${name}`,
    method: "GET",
    signal,
  });
};

export const getGetPoolQueryKey = (name: string) => {
  return [`/api/v1/pools/${name}`] as const;
};

export const getGetPoolQueryOptions = <
  TData = Awaited<ReturnType<typeof getPool>>,
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse,
>(
  name: string,
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getPool>>, TError, TData>
    >;
  },
) => {
  const { query: queryOptions } = options ?? {};

  const queryKey = queryOptions?.queryKey ?? getGetPoolQueryKey(name);

  const queryFn: QueryFunction<Awaited<ReturnType<typeof getPool>>> = ({
    signal,
  }) => getPool(name, signal);

  return {
    queryKey,
    queryFn,
    enabled: name !== null && name !== undefined,
    ...queryOptions,
  } as UseQueryOptions<Awaited<ReturnType<typeof getPool>>, TError, TData> & {
    queryKey: DataTag<QueryKey, TData, TError>;
  };
};

export type GetPoolQueryResult = NonNullable<
  Awaited<ReturnType<typeof getPool>>
>;
export type GetPoolQueryError =
  | BadRequestResponse
  | UnauthorizedResponse
  | ForbiddenResponse
  | NotFoundResponse;

export function useGetPool<
  TData = Awaited<ReturnType<typeof getPool>>,
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse,
>(
  name: string,
  options: {
    query: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getPool>>, TError, TData>
    > &
      Pick<
        DefinedInitialDataOptions<
          Awaited<ReturnType<typeof getPool>>,
          TError,
          Awaited<ReturnType<typeof getPool>>
        >,
        "initialData"
      >;
  },
  queryClient?: QueryClient,
): DefinedUseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useGetPool<
  TData = Awaited<ReturnType<typeof getPool>>,
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse,
>(
  name: string,
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getPool>>, TError, TData>
    > &
      Pick<
        UndefinedInitialDataOptions<
          Awaited<ReturnType<typeof getPool>>,
          TError,
          Awaited<ReturnType<typeof getPool>>
        >,
        "initialData"
      >;
  },
  queryClient?: QueryClient,
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
export function useGetPool<
  TData = Awaited<ReturnType<typeof getPool>>,
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse,
>(
  name: string,
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getPool>>, TError, TData>
    >;
  },
  queryClient?: QueryClient,
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
};
/**
 * @summary Get pool detail
 */

export function useGetPool<
  TData = Awaited<ReturnType<typeof getPool>>,
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse,
>(
  name: string,
  options?: {
    query?: Partial<
      UseQueryOptions<Awaited<ReturnType<typeof getPool>>, TError, TData>
    >;
  },
  queryClient?: QueryClient,
): UseQueryResult<TData, TError> & {
  queryKey: DataTag<QueryKey, TData, TError>;
} {
  const queryOptions = getGetPoolQueryOptions(name, options);

  const query = useQuery(queryOptions, queryClient) as UseQueryResult<
    TData,
    TError
  > & { queryKey: DataTag<QueryKey, TData, TError> };

  return withQueryKey(query, queryOptions.queryKey);
}

/**
 * @summary Create a worker pool
 */
export const createPool = (
  name: string,
  createPoolBody: CreatePoolBody,
  signal?: AbortSignal,
) => {
  return apiClient<PoolMutation>({
    url: `/api/v1/pools/${name}`,
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    data: createPoolBody,
    signal,
  });
};

export const getCreatePoolMutationKey = () => ["createPool"] as const;

export const getCreatePoolMutationOptions = <
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | ConflictResponse,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof createPool>>,
    TError,
    CreatePoolMutationVariables,
    TContext
  >;
}): UseMutationOptions<
  Awaited<ReturnType<typeof createPool>>,
  TError,
  CreatePoolMutationVariables,
  TContext
> => {
  const mutationKey = getCreatePoolMutationKey();
  const { mutation: mutationOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey } };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof createPool>>,
    CreatePoolMutationVariables
  > = (props) => {
    const { name, data } = props ?? {};

    return createPool(name, data);
  };

  return { mutationFn, ...mutationOptions };
};

export type CreatePoolMutationResult = NonNullable<
  Awaited<ReturnType<typeof createPool>>
>;
export type CreatePoolMutationBody = CreatePoolBody;
export type CreatePoolMutationError =
  | BadRequestResponse
  | UnauthorizedResponse
  | ForbiddenResponse
  | ConflictResponse;
export type CreatePoolMutationVariables = {
  name: string;
  data: CreatePoolBody;
};

/**
 * @summary Create a worker pool
 */
export const useCreatePool = <
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | ConflictResponse,
  TContext = unknown,
>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof createPool>>,
      TError,
      CreatePoolMutationVariables,
      TContext
    >;
  },
  queryClient?: QueryClient,
): UseMutationResult<
  Awaited<ReturnType<typeof createPool>>,
  TError,
  CreatePoolMutationVariables,
  TContext
> => {
  return useMutation(getCreatePoolMutationOptions(options), queryClient);
};
/**
 * @summary Update a worker pool
 */
export const updatePool = (
  name: string,
  updatePoolBody: UpdatePoolBody,
  signal?: AbortSignal,
) => {
  return apiClient<PoolMutation>({
    url: `/api/v1/pools/${name}`,
    method: "PATCH",
    headers: { "Content-Type": "application/json" },
    data: updatePoolBody,
    signal,
  });
};

export const getUpdatePoolMutationKey = () => ["updatePool"] as const;

export const getUpdatePoolMutationOptions = <
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse
    | ConflictResponse,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof updatePool>>,
    TError,
    UpdatePoolMutationVariables,
    TContext
  >;
}): UseMutationOptions<
  Awaited<ReturnType<typeof updatePool>>,
  TError,
  UpdatePoolMutationVariables,
  TContext
> => {
  const mutationKey = getUpdatePoolMutationKey();
  const { mutation: mutationOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey } };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof updatePool>>,
    UpdatePoolMutationVariables
  > = (props) => {
    const { name, data } = props ?? {};

    return updatePool(name, data);
  };

  return { mutationFn, ...mutationOptions };
};

export type UpdatePoolMutationResult = NonNullable<
  Awaited<ReturnType<typeof updatePool>>
>;
export type UpdatePoolMutationBody = UpdatePoolBody;
export type UpdatePoolMutationError =
  | BadRequestResponse
  | UnauthorizedResponse
  | ForbiddenResponse
  | NotFoundResponse
  | ConflictResponse;
export type UpdatePoolMutationVariables = {
  name: string;
  data: UpdatePoolBody;
};

/**
 * @summary Update a worker pool
 */
export const useUpdatePool = <
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse
    | ConflictResponse,
  TContext = unknown,
>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof updatePool>>,
      TError,
      UpdatePoolMutationVariables,
      TContext
    >;
  },
  queryClient?: QueryClient,
): UseMutationResult<
  Awaited<ReturnType<typeof updatePool>>,
  TError,
  UpdatePoolMutationVariables,
  TContext
> => {
  return useMutation(getUpdatePoolMutationOptions(options), queryClient);
};
/**
 * @summary Delete a worker pool
 */
export const deletePool = (
  name: string,
  params?: DeletePoolParams,
  signal?: AbortSignal,
) => {
  return apiClient<void>({
    url: `/api/v1/pools/${name}`,
    method: "DELETE",
    params,
    signal,
  });
};

export const getDeletePoolMutationKey = () => ["deletePool"] as const;

export const getDeletePoolMutationOptions = <
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse
    | ConflictResponse,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof deletePool>>,
    TError,
    DeletePoolMutationVariables,
    TContext
  >;
}): UseMutationOptions<
  Awaited<ReturnType<typeof deletePool>>,
  TError,
  DeletePoolMutationVariables,
  TContext
> => {
  const mutationKey = getDeletePoolMutationKey();
  const { mutation: mutationOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey } };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof deletePool>>,
    DeletePoolMutationVariables
  > = (props) => {
    const { name, params } = props ?? {};

    return deletePool(name, params);
  };

  return { mutationFn, ...mutationOptions };
};

export type DeletePoolMutationResult = NonNullable<
  Awaited<ReturnType<typeof deletePool>>
>;

export type DeletePoolMutationError =
  | BadRequestResponse
  | UnauthorizedResponse
  | ForbiddenResponse
  | NotFoundResponse
  | ConflictResponse;
export type DeletePoolMutationVariables = {
  name: string;
  params?: DeletePoolParams;
};

/**
 * @summary Delete a worker pool
 */
export const useDeletePool = <
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse
    | ConflictResponse,
  TContext = unknown,
>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof deletePool>>,
      TError,
      DeletePoolMutationVariables,
      TContext
    >;
  },
  queryClient?: QueryClient,
): UseMutationResult<
  Awaited<ReturnType<typeof deletePool>>,
  TError,
  DeletePoolMutationVariables,
  TContext
> => {
  return useMutation(getDeletePoolMutationOptions(options), queryClient);
};
/**
 * @summary Mark a pool as draining
 */
export const drainPool = (
  name: string,
  drainPoolBody?: DrainPoolBody,
  signal?: AbortSignal,
) => {
  return apiClient<PoolMutation>({
    url: `/api/v1/pools/${name}/drain`,
    method: "POST",
    headers: { "Content-Type": "application/json" },
    data: drainPoolBody,
    signal,
  });
};

export const getDrainPoolMutationKey = () => ["drainPool"] as const;

export const getDrainPoolMutationOptions = <
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse
    | ConflictResponse,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof drainPool>>,
    TError,
    DrainPoolMutationVariables,
    TContext
  >;
}): UseMutationOptions<
  Awaited<ReturnType<typeof drainPool>>,
  TError,
  DrainPoolMutationVariables,
  TContext
> => {
  const mutationKey = getDrainPoolMutationKey();
  const { mutation: mutationOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey } };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof drainPool>>,
    DrainPoolMutationVariables
  > = (props) => {
    const { name, data } = props ?? {};

    return drainPool(name, data);
  };

  return { mutationFn, ...mutationOptions };
};

export type DrainPoolMutationResult = NonNullable<
  Awaited<ReturnType<typeof drainPool>>
>;
export type DrainPoolMutationBody = DrainPoolBody | undefined;
export type DrainPoolMutationError =
  | BadRequestResponse
  | UnauthorizedResponse
  | ForbiddenResponse
  | NotFoundResponse
  | ConflictResponse;
export type DrainPoolMutationVariables = { name: string; data?: DrainPoolBody };

/**
 * @summary Mark a pool as draining
 */
export const useDrainPool = <
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse
    | ConflictResponse,
  TContext = unknown,
>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof drainPool>>,
      TError,
      DrainPoolMutationVariables,
      TContext
    >;
  },
  queryClient?: QueryClient,
): UseMutationResult<
  Awaited<ReturnType<typeof drainPool>>,
  TError,
  DrainPoolMutationVariables,
  TContext
> => {
  return useMutation(getDrainPoolMutationOptions(options), queryClient);
};
/**
 * @summary Add a topic mapping to a pool
 */
export const addTopicToPool = (
  name: string,
  topic: string,
  signal?: AbortSignal,
) => {
  return apiClient<void>({
    url: `/api/v1/pools/${name}/topics/${topic}`,
    method: "PUT",
    signal,
  });
};

export const getAddTopicToPoolMutationKey = () => ["addTopicToPool"] as const;

export const getAddTopicToPoolMutationOptions = <
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse
    | ConflictResponse,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof addTopicToPool>>,
    TError,
    AddTopicToPoolMutationVariables,
    TContext
  >;
}): UseMutationOptions<
  Awaited<ReturnType<typeof addTopicToPool>>,
  TError,
  AddTopicToPoolMutationVariables,
  TContext
> => {
  const mutationKey = getAddTopicToPoolMutationKey();
  const { mutation: mutationOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey } };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof addTopicToPool>>,
    AddTopicToPoolMutationVariables
  > = (props) => {
    const { name, topic } = props ?? {};

    return addTopicToPool(name, topic);
  };

  return { mutationFn, ...mutationOptions };
};

export type AddTopicToPoolMutationResult = NonNullable<
  Awaited<ReturnType<typeof addTopicToPool>>
>;

export type AddTopicToPoolMutationError =
  | BadRequestResponse
  | UnauthorizedResponse
  | ForbiddenResponse
  | NotFoundResponse
  | ConflictResponse;
export type AddTopicToPoolMutationVariables = { name: string; topic: string };

/**
 * @summary Add a topic mapping to a pool
 */
export const useAddTopicToPool = <
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse
    | ConflictResponse,
  TContext = unknown,
>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof addTopicToPool>>,
      TError,
      AddTopicToPoolMutationVariables,
      TContext
    >;
  },
  queryClient?: QueryClient,
): UseMutationResult<
  Awaited<ReturnType<typeof addTopicToPool>>,
  TError,
  AddTopicToPoolMutationVariables,
  TContext
> => {
  return useMutation(getAddTopicToPoolMutationOptions(options), queryClient);
};
/**
 * @summary Remove a topic mapping from a pool
 */
export const removeTopicFromPool = (
  name: string,
  topic: string,
  signal?: AbortSignal,
) => {
  return apiClient<void>({
    url: `/api/v1/pools/${name}/topics/${topic}`,
    method: "DELETE",
    signal,
  });
};

export const getRemoveTopicFromPoolMutationKey = () =>
  ["removeTopicFromPool"] as const;

export const getRemoveTopicFromPoolMutationOptions = <
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse
    | ConflictResponse,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof removeTopicFromPool>>,
    TError,
    RemoveTopicFromPoolMutationVariables,
    TContext
  >;
}): UseMutationOptions<
  Awaited<ReturnType<typeof removeTopicFromPool>>,
  TError,
  RemoveTopicFromPoolMutationVariables,
  TContext
> => {
  const mutationKey = getRemoveTopicFromPoolMutationKey();
  const { mutation: mutationOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
      ? options
      : { ...options, mutation: { ...options.mutation, mutationKey } }
    : { mutation: { mutationKey } };

  const mutationFn: MutationFunction<
    Awaited<ReturnType<typeof removeTopicFromPool>>,
    RemoveTopicFromPoolMutationVariables
  > = (props) => {
    const { name, topic } = props ?? {};

    return removeTopicFromPool(name, topic);
  };

  return { mutationFn, ...mutationOptions };
};

export type RemoveTopicFromPoolMutationResult = NonNullable<
  Awaited<ReturnType<typeof removeTopicFromPool>>
>;

export type RemoveTopicFromPoolMutationError =
  | BadRequestResponse
  | UnauthorizedResponse
  | ForbiddenResponse
  | NotFoundResponse
  | ConflictResponse;
export type RemoveTopicFromPoolMutationVariables = {
  name: string;
  topic: string;
};

/**
 * @summary Remove a topic mapping from a pool
 */
export const useRemoveTopicFromPool = <
  TError =
    | BadRequestResponse
    | UnauthorizedResponse
    | ForbiddenResponse
    | NotFoundResponse
    | ConflictResponse,
  TContext = unknown,
>(
  options?: {
    mutation?: UseMutationOptions<
      Awaited<ReturnType<typeof removeTopicFromPool>>,
      TError,
      RemoveTopicFromPoolMutationVariables,
      TContext
    >;
  },
  queryClient?: QueryClient,
): UseMutationResult<
  Awaited<ReturnType<typeof removeTopicFromPool>>,
  TError,
  RemoveTopicFromPoolMutationVariables,
  TContext
> => {
  return useMutation(
    getRemoveTopicFromPoolMutationOptions(options),
    queryClient,
  );
};
