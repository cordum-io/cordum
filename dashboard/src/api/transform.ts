...
const FRAMEWORK_TAG_ALIASES: Record<string, FrameworkTag> = {
  soc2: "soc2",
  soc_2: "soc2",
  "soc-2": "soc2",
  iso27001: "iso27001",
  ...
  nist: "nist-rmf",
  gdpr: "gdpr",
  "owasp-llm-top10": "owasp-llm-top10",
...
function normalizeFrameworkTag(raw: unknown): FrameworkTag | null {
  if (typeof raw !== "string") return null;
  const trimmed = raw.trim();
  if (!trimmed) return null;
  const normalizedKey = trimmed.toLowerCase();
  return FRAMEWORK_TAG_ALIASES[normalizedKey] ?? normalizedKey;
}

function normalizeFrameworkTags(raw: unknown): FrameworkTag[] | undefined {
  if (!Array.isArray(raw)) return undefined;
  const normalized = raw
    .map(normalizeFrameworkTag)
    .filter((tag): tag is FrameworkTag => Boolean(tag));
...
export function mapPolicyRule(raw: Record<string, unknown>): PolicyRule {
  ...
  return {
    ...
    priority,
    logic,
    hitCount24h: typeof raw.hit_count_24h === "number" ? raw.hit_count_24h : (typeof raw.hitCount24h === "number" ? raw.hitCount24h : undefined),
    lastTriggered: typeof raw.last_triggered === "string" ? raw.last_triggered : (typeof raw.lastTriggered === "string" ? raw.lastTriggered : undefined),
    frameworkTags: normalizeFrameworkTags(raw.framework_tags) ?? normalizeFrameworkTags(raw.frameworkTags),
...
function mapJobEventContext(
  raw: Record<string, unknown> | undefined,
): JobEventContext | undefined {
  if (!raw) return undefined;
  return {
    rule: typeof raw.rule === "string" ? raw.rule : undefined,
    reason: typeof raw.reason === "string" ? raw.reason : undefined,
    evalMs: typeof raw.eval_ms === "number" ? raw.eval_ms : undefined,
    evalPath: Array.isArray(raw.eval_path) ? raw.eval_path : undefined,
    workerId: typeof raw.worker_id === "string" ? raw.worker_id : undefined,
    pool: typeof raw.pool === "string" ? raw.pool : undefined,
    strategy: typeof raw.strategy === "string" ? raw.strategy : undefined,
    approvedBy: typeof raw.approved_by === "string" ? raw.approved_by : undefined,
    approvalRole: typeof raw.approval_role === "string" ? raw.approval_role : undefined,
    approvalNote: typeof raw.approval_note === "string" ? raw.approval_note : undefined,
    waitMs: typeof raw.wait_ms === "number" ? raw.wait_ms : undefined,
    errorCode: typeof raw.error_code === "string" ? raw.error_code : undefined,
    errorMsg: typeof raw.error_msg === "string" ? raw.error_msg : undefined,
    resultPtr: typeof raw.result_ptr === "string" ? raw.result_ptr : undefined,
    durationMs: typeof raw.duration_ms === "number" ? raw.duration_ms : undefined,
    findings: typeof raw.findings === "number" ? raw.findings : undefined,
    ...
    riskTags: Array.isArray(raw.risk_tags) ? raw.risk_tags : undefined,
    actorId: typeof raw.actor_id === "string" ? raw.actor_id : undefined,
  };
}

export function mapJobEvent(raw: Record<string, unknown>): JobEvent {
  return {
    timestamp: microsToISO(raw.ts),
    state: typeof raw.state === "string" ? raw.state : "",
    ctx: mapJobEventContext(raw.ctx as Record<string, unknown> | undefined),
  };
}

const stateToKind: Record<string, TimelineEventKind> = {
  PENDING: "submitted",
...
function resolveKind(state: string, ctx?: JobEventContext): TimelineEventKind {
  const upper = state.toUpperCase();
  if (upper === "SCHEDULED" && ctx?.approvedBy) return "approved";
  return stateToKind[upper] ?? "submitted";
}

function resolveTitle(kind: TimelineEventKind, ctx?: JobEventContext): string {
  switch (kind) {
    case "submitted":
      return ctx?.topic ? `Submitted to ${ctx.topic}` : "Submitted";
    case "policy_evaluated":
      return ctx?.evalMs != null ? `Policy Evaluated (${ctx.evalMs}ms)` : "Policy Evaluated";
    case "approval_requested":
      return "Approval Requested";
    case "approved":
      return ctx?.approvedBy ? `Approved by ${ctx.approvedBy}` : "Approved";
    case "rejected":
      return ctx?.reason ? `Rejected: ${ctx.reason}` : "Rejected";
    case "dispatched":
      return ctx?.workerId ? `Dispatched to ${ctx.workerId}` : "Dispatched";
    case "executing":
      return "Executing";
    case "progress":
      return "Progress Update";
    case "output_scanned":
      return ctx?.findings != null ? `Output Scanned (${ctx.findings} findings)` : "Output Scanned";
    case "succeeded":
      return ctx?.durationMs != null ? `Succeeded (${ctx.durationMs}ms)` : "Succeeded";
    case "failed":
      return ctx?.errorMsg ? `Failed: ${ctx.errorMsg}` : "Failed";
    case "denied":
      return ctx?.reason ? `Denied: ${ctx.reason}` : "Denied";
    case "timeout":
      return "Timed Out";
    case "cancelled":
      return "Cancelled";
    case "quarantined":
      return ctx?.scanner ? `Quarantined by ${ctx.scanner}` : "Quarantined";
  }
}

function resolveDetail(ctx?: JobEventContext): string | undefined {
  if (!ctx) return undefined;
  const parts: string[] = [];
  if (ctx.rule) parts.push(`Rule: ${ctx.rule}`);
  if (ctx.pool) parts.push(`Pool: ${ctx.pool}`);
  if (ctx.strategy) parts.push(`Strategy: ${ctx.strategy}`);
  if (ctx.errorCode) parts.push(`Code: ${ctx.errorCode}`);
  return parts.length > 0 ? parts.join(" · ") : undefined;
}

const errorKinds = new Set<TimelineEventKind>(["failed", "denied", "timeout"]);
const warningKinds = new Set<TimelineEventKind>(["approval_requested"]);
...
function resolveStatus(
  ...
  if (errorKinds.has(kind)) return "error";
  if (warningKinds.has(kind)) return "warning";
  if (index === lastIndex) return "current";
  return "completed";
}

export function buildUnifiedTimeline(events: JobEvent[], job: Job): TimelineEvent[] {
  // Zero-event fallback: derive minimal timeline from job fields
  if (events.length === 0) {
    const fallback: TimelineEvent[] = [];
    const submittedCtx: JobEventContext = {
      topic: job.topic,
      capabilities: job.capabilities,
      riskTags: job.riskTags,
    };
    fallback.push({
      ...
      kind: "submitted",
      title: job.topic ? `Submitted to ${job.topic}` : "Submitted",
      detail: resolveDetail(submittedCtx),
      expandedData: submittedCtx,
      status: job.status === "pending" ? "current" : "completed",
    });

    if (job.status !== "pending") {
      const currentKind = stateToKind[job.status.toUpperCase()] ?? "submitted";
      const currentCtx: JobEventContext = {
      ...
      fallback.push({
        ...
        kind: currentKind,
        title: resolveTitle(currentKind, currentCtx),
        detail: resolveDetail(currentCtx),
        expandedData: currentCtx,
        status: errorKinds.has(currentKind) ? "error" : "current",
      });
    }
    return fallback;
  }
  ...
  return events.map((evt, i) => {
    const kind = resolveKind(evt.state, evt.ctx);
    return {
      timestamp: evt.timestamp,
      kind,
      title: resolveTitle(kind, evt.ctx),
      detail: resolveDetail(evt.ctx),
      expandedData: evt.ctx,
      status: resolveStatus(kind, i, lastIndex, job.status),
    };
  });
}
