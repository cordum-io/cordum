...
import { isOutputQuarantinedEntry } from "../../lib/dlqSignals";
import { cn } from "../../lib/utils";

// ---------------------------------------------------------------------------
// DLQ Summary — compact widget for the Command Center / Failures / Quarantine
// ---------------------------------------------------------------------------

export function DLQSummary({ variant }: { variant?: "default" | "quarantine" } = {}) {
  ...
  if (isLoading) {
    return (
      <Card variant={variant === "quarantine" ? "danger" : "default"}>
        <CardHeader>
  ...
  if (entries.length === 0) {
    return (
      <Card variant={variant === "quarantine" ? "default" : "default"}>
        <CardHeader>
          <CardTitle className="text-sm">{title}</CardTitle>
        </CardHeader>
        <div className="flex items-center gap-2 text-sm text-success">
          <CheckCircle className="h-4 w-4" />
          No {variant === "quarantine" ? "quarantined outputs" : "failed jobs"}
        </div>
      </Card>
    );
  }
  ...
  const reasons = entries
    .slice(0, 3)
    .map((e) => e.error || e.reason || e.reasonCode || "Unknown error");

  return (
    <Card variant={variant === "quarantine" ? "danger" : "default"}>
      <CardHeader>
        <div className="flex items-center gap-2">
          <Icon className={cn("h-4 w-4", variant === "quarantine" ? "text-danger" : "text-danger")} />
          <CardTitle className="text-sm">{title}</CardTitle>
        </div>
