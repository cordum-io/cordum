...
describe("ComboboxInput", () => {
  ...
  it("calls onChange when suggestion is clicked", () => {
    ...
    const input = getInput();
    act(() => input.focus());
    const firstItem = container.querySelector("li button");
    act(() => {
  ...
  it("navigates suggestions with arrow keys and selects with Enter", () => {
    ...
    // First item button should be highlighted (activeIdx = 0)
    const items = container.querySelectorAll("li button");
    expect(items[0].className).toContain("bg-status-info-bg");
