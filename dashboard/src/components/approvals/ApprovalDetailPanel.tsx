...
import { InvestigationLinks } from "../InvestigationLinks";
...
export function ApprovalDetailPanel({
  ...
  const dialogTitleId = `approval-detail-title-${approval.id}`;
  const dialogMetaId = `approval-detail-meta-${approval.id}`;
  const conditionTextId = `approval-condition-${approval.id}`;
  const modifiedPayloadId = `approval-modified-payload-${approval.id}`;
  const payloadErrorId = `${modifiedPayloadId}-error`;
  const isFeedbackError = Boolean(feedback && (feedback.startsWith("Action failed") || feedback.startsWith("This item is no longer")));
    ...
    <>
      ...
      >
        ...
        <div className="sticky top-0 z-10 space-y-3 border-b border-border bg-surface/95 backdrop-blur px-6 py-4">
          <div className="flex items-start justify-between">
            <div className="space-y-1 min-w-0 flex-1">
              <p id={dialogTitleId} className="text-xl font-semibold text-ink leading-tight truncate">
                {approval.humanSummary || `Job ${approval.jobId.slice(0, 8)} requires approval`}
              </p>
              <div id={dialogMetaId} className="flex items-center gap-2 text-xs text-muted">
            ...
            <button
              type="button"
              onClick={onClose}
              className="ml-3 rounded p-1.5 text-muted transition-colors hover:bg-surface2 hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/35"
          ...
          {feedback && (
            <p
              ...
              aria-live={isFeedbackError ? "assertive" : "polite"}
              className={cn("text-sm font-medium", isFeedbackError ? "text-warning" : "text-success")}
            >
              {feedback}
            </p>
          )}
          ...
          {mode === "confirming-conditions" && (
            <div className="space-y-3">
              ...
              <div className="space-y-1">
                <label htmlFor={conditionTextId} className="text-xs font-medium text-muted">Condition (required)</label>
                <Textarea
                  id={conditionTextId}
                  rows={3}
              ...
              {/* Modified payload */}
              <div className="space-y-1">
                <label htmlFor={modifiedPayloadId} className="text-xs font-medium text-muted">Modify payload (optional)</label>
                <textarea
                  id={modifiedPayloadId}
                  rows={6}
                  className={cn(
                ...
                />
                {payloadError && (
                  <p id={payloadErrorId} className="text-xs text-danger">{payloadError}</p>
                )}
              </div>

              {/* Scope radio */}
              <fieldset className="space-y-1">
                <legend className="text-xs font-medium text-muted">Scope</legend>
                <div className="flex items-center gap-4">
                  <label htmlFor="condition-scope-this-run" className="flex items-center gap-1.5 text-xs text-ink cursor-pointer">
                    <input
                      id="condition-scope-this-run"
                      type="radio"
                  ...
                  <label htmlFor="condition-scope-pattern" className="flex items-center gap-1.5 text-xs text-ink cursor-pointer">
                    <input
                      id="condition-scope-pattern"
                      type="radio"
                      name="condition-scope"
                      className="accent-accent"
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-6">
          {/* Section 1.5: Decision Rationale */}
          <div className="relative overflow-hidden rounded-xl border border-warning/30 bg-warning/5 p-5">
            ...
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1">
                <h4 className="text-xs font-bold uppercase tracking-wider text-warning/80">Decision Required</h4>
                <p className="text-base font-semibold text-ink leading-relaxed">
                  {approval.humanSummary ? (
                    approval.humanSummary
                  ) : (
                    <>
                      Requires approval because rule <span className="text-warning">{approval.policyRule || "unknown"}</span> matched
                      {approval.riskTags?.length ? (
                        <> with risk tags: <span className="italic">{approval.riskTags.join(", ")}</span></>
                      ) : "."}
                    </>
                  )}
                </p>
          ...
          {/* Section 3.5: Investigation Links */}
          <Section title="Investigation">
            <InvestigationLinks
              type="approval"
              jobId={approval.jobId}
              ruleId={approval.policyRule}
            />
          </Section>
