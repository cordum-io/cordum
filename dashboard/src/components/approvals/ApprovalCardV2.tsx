...
import { APPROVER_ROLES } from "../../hooks/usePermission";
...
const urgencyBorderColor: Record<UrgencyLevel, string> = {
  ...
  aging: "border-l-warning",
  critical: "border-l-danger",
  breach: "border-l-danger animate-pulse",
};
...
export function ApprovalCardV2({
  ...
  return (
    <Card
      className={cn(
        "border-l-4 p-3.5 transition-shadow hover:shadow-lift",
      ...
      <div className="flex gap-3">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {breach && (
              <span className="rounded bg-danger px-1.5 py-0.5 text-[10px] font-bold uppercase text-surface-0">
                SLA Breach
              </span>
            )}
          </div>
        ...
        {(presence || assignee) && (
          <div className="flex flex-wrap items-center gap-2">
            {presence && (
              <span className="flex items-center gap-1 rounded-md border border-status-info-border bg-status-info-bg px-2 py-0.5 text-[10px] font-medium text-info">
                <Eye className="h-3 w-3" />
                Being reviewed by @{presence.actor}
              </span>
            )}
            {assignee && (
              <span className="flex items-center gap-1 rounded-md border border-border/70 bg-surface-2/60 px-2 py-0.5 text-[10px] font-medium text-foreground">
        ...
        <div className="space-y-0.5 text-xs text-muted-foreground">
          {approval.policyRule && (
            <p>
              Rule:{" "}
              <span className="font-medium text-foreground">{approval.policyRule}</span>
        ...
        <RequireRole 
          ...
          fallback={
            <div className="flex flex-wrap items-center justify-end gap-2 pt-1">
              <span className="text-[10px] font-bold uppercase tracking-tight text-muted/60">Read-only</span>
              <Button
                variant="outline"
                size="sm"
        ...
        >
          {mode === "idle" && (
            <div className="flex flex-wrap items-center justify-end gap-2 pt-1">
              <Button
                size="sm"
                className="bg-success text-surface-0 hover:brightness-110"
