...
| Govern | `pages/policies/DecisionsPage.tsx` | PageHeader, DataTable (+expand-row), Select, Button, StatusBadge, EmptyState, nuqs URL state, CodeBlock, Recharts (lazy), Drawer, RuleMonacoEditor (lazy) | DONE (Dashboard 8a task-18957027 + Dashboard 8b task-66f9abcb + Dashboard 9b task-e343469b — page shell, filter bar, paged table, expand-row, live mode + 1Hz throttle, cursor pagination, Charts panel with 4 Recharts charts, Replay action wired to POST /policy/replay, What-if drawer with Monaco edit + re-evaluate / no-save) |
