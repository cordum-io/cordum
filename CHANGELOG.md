...
- Added `core/policy/legacy_to_unified.go` — single-entry translator `LegacyMapToUnifiedRule(legacy map[string]any, ruleType RuleType, source *PackSource)` that consumes the YAML-derived rule maps `policybundles.RulesFromPolicyContent` and `OutputRulesFromPolicyContent` produce. Field mapping: id (required); name (id-fallback); description (reason-fallback); tier+selector → `Scope`; status default `published`; match preserves payload + embeds top-level `velocity` block under `match.velocity` for Type=velocity; decide synthesizes from decision+reason+severity; `Audit.PackSource` populated from caller-supplied source.
- Added `core/policy.PackSource` struct mirroring `policybundles.PolicyRuleSource` field-for-field (fragment_id, pack_id, overlay_name, tier, version, installed_at, sha256). Extended `core/policy.AuditMetadata` with optional `PackSource *PackSource` (omitempty) so the dashboard's pack_id + tier columns + Source filter survive the legacy → unified migration losslessly. RuleStore-authored rules (interactive Dashboard 3E publish) leave `pack_source` unset.
- Added `core/controlplane/gateway/handlers_policy_rules_unified.go::handlePolicyRulesUnified`. Sources merged in order: (1) `RuleStore.ListRules` (interactive rules, Backend 5c); (2) YAML input bundles → Type=input; (3) YAML output bundles → Type=output; (4) Velocity bundles → Type=velocity. Filter params: `type` (RuleType), `scope_kind` (RuleScopeKind), `scope_value`, `status` (RuleStatus), `pack_id`, `limit` (1-500, default 100), `cursor` (opaque base64 of last id). All filters AND-combined; sorted by ID ASC for deterministic cursor; 400 on invalid params; empty store returns 200 `{items: [], has_more: false}`.
- Extended `core/policy.RuleStore` with `ListRules(ctx) ([]*Rule, error)` so the unified handler can enumerate all RuleStore rules without scope round-trips. `RuleRedisStore.ListRules` SCANs `policy:rule:*` keys filtering out `:version` sidecars; sorts by ID.
- Replaced legacy `handlePolicyRules` body with thin alias delegating to `handlePolicyRulesUnified`. Route registration unchanged.
- Bumped `cordum-api.yaml` to `2026-05-10.3` with the new `RulesListResponse` + `PackSource` schemas + 7 new query params on `GET /api/v1/policy/rules` + `pack_source` on `AuditMetadata`. Marked `GET /api/v1/policy/output/rules` and `GET /api/v1/policy/velocity-rules` as `deprecated: true` with cross-ref to D11 cut-over (the legacy paths stay alive until then).
- Dashboard regen via `pnpm run generate-api` emits `RulesListResponse` + `PackSource` model files; `listPolicyRules()` now returns `RulesListResponse`. Additions-only diff against branch tip.
- MSW: new `dashboard/src/test-utils/fixtures/rules-unified.ts` with 8-row fixture mirroring the dev-stack data shape (cordclaw/openclaw/visa/b2b/claude-code packs across all 4 RuleType values). Default GET `/api/v1/policy/rules` MSW handler updated to the new envelope shape `{items: [], has_more: false, next_cursor: ""}`; tests opt into the populated fixture via `server.use(...)`.
- 18 new tests: 8 in `core/policy/legacy_to_unified_test.go` (translation contracts) + 8 in `handlers_policy_rules_unified_test.go` (filters / pagination / RuleStore merge / empty / legacy preserved) + 1 in `handlers_policy_rules_source_test.go` (all 6 pack-source fields lossless) + 1 in handler unified test for legacy regression. -count=3 race-clean on policy package.
- Live BEFORE smoke captured: `curl https://localhost:8081/api/v1/policy/rules` against the running dev stack (pre-5d binary) returned the legacy `{decision, id, match, reason, source, tier}` shape with no `type` field — exactly the bug being fixed. Live AFTER smoke is post-merge once the api-gateway container is rebuilt with the new binary.


#### Policy Studio Backend 5c — unified Rule write API + add-rule-to-bundle endpoint (2026-05-10, task-710b40cf)

- Added `RuleStore` interface (`core/policy/rule_store.go`) with `CreateRule`/`GetRule`/`UpdateRule`/`DeleteRule`/`ListRulesByScope`. Introduced typed `*ErrRuleStaleVersion{CurrentVersion, CurrentAuditHash}` + `IsStaleVersionError(err)` helper so handlers can map stale-If-Match conflicts to `409 + body` without sniffing strings. Sentinel errors `ErrRuleNotFound`/`ErrRuleExists`/`ErrRuleValidation`.
- `RuleRedisStore` (`core/policy/rule_store_redis.go`) splits envelope + version into separate keys (`policy:rule:{id}` STRING for the JSON envelope, `policy:rule:{id}:version` STRING sidecar for CAS) so the Lua script compares simple strings — miniredis `cjson.decode` is unreliable on nested objects, and the sidecar makes the CAS race-clean across miniredis and real Redis. Cross-scope rebalance via SREM/SADD outside Lua (idempotent on retry; race-acceptable per architect ack).
- Server-side metadata is the source of truth: `applyServerSideCreateMetadata` sets `Version=v1` + `Audit.CreatedAt/UpdatedAt` + `Status=draft`; `applyServerSideUpdateMetadata` bumps `vN -> v(N+1)` + preserves prior `Status` when the request body's status is empty. Clients cannot fake history.
- Added `BundleStore.AddRuleToBundle(bundleID, ruleID, ruleExists)` (`core/policy/bundle_store.go` + Redis impl) — read-then-CAS-write Lua pair retries on conflict so concurrent calls with distinct rule IDs converge without lost writes. Idempotent on duplicate adds. The `ruleExists` callback decouples BundleStore from RuleStore; the gateway closes over `RuleStore.GetRule` to inject existence checks without a tight binding. Architect's load-bearing race test (`TestAddRuleToBundleConcurrentDistinctIDs`, 10 goroutines / distinct ruleIDs / no lost writes) passes at `-count=3`.
- Added 3 gateway endpoints (`core/controlplane/gateway/handlers_policy_rules.go` + `handlers_policy_bundles_rules.go`):
  - `POST /api/v1/policy/rules` — creates a Rule; rejects client-set `version`/`audit.*` with 400; duplicate ID → 409; returns 201 + `Location` header + persisted Rule.
  - `PUT /api/v1/policy/rules/{id}` — optimistic concurrency via `If-Match: <Rule.Version>`. Missing header → 412; stale → 409 with body `{"error":"stale_version","current_version":"vN","current_audit_hash":"..."}` so the dashboard renders a reload banner without re-fetching. Path id wins over body id on conflict.
  - `POST /api/v1/policy/bundles/{id}/rules` — binds an existing rule into a bundle's `rule_ids` set, idempotent on repeat. Two 404 paths disambiguated as `error: "rule_not_found"` vs `error: "bundle_not_found"` so the dashboard can present the right copy.
- All three handlers gate on `auth.PermPolicyWrite` + admin role mirror.
- Bumped `cordum-api.yaml` to `2026-05-10.2` with the 3 new paths, `RuleStaleVersionResponse` schema, `AddRuleToBundleRequest` schema, and the inline disambiguated-404 schema for the binding endpoint. Ran `pnpm run generate-api` to emit `createPolicyRule`/`updatePolicyRule`/`addRuleToBundle` orval hooks. Dashboard MSW handlers (`test-utils/handlers.ts`) cover the 201 / 200 / 404+`bundle_not_found` / 404+`rule_not_found` / 409+`stale_version` / 428+`If-Match-missing` paths so D3E reload-banner tests run without per-test setup; `fixtures/rules.ts` ships two canonical rules (tenant-scoped input + global-scoped edge).
- Deferred: orval doesn't auto-emit header parameters, so dashboard PUT callers will need a thin wrapper that calls `apiClient` with `If-Match` directly. Tracked as a Backend 5d follow-up.
