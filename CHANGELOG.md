...
- Three entangled hardenings of the Phase-2 session-token gate (`CORDUM_SDK_HANDSHAKE`) so it can run in `enforce`:
  - **Internal-broadcast auth (CodeRabbit PR #326 #7).** Under enforce, peer schedulers dropped the control plane's OWN internal `JobResult`/`JobCancel` broadcasts (they carried no auth token), so a cancel flipped the local job store but never propagated across schedulers. Fixed with a stateless, short-TTL (~5m) Ed25519 **service token** (`typ=cordum-service`, synthetic `_system` tenant) minted by each producer with the signing key it already holds and verified by a peer with `checkActive=false` — no Redis active record (a service token is unrevocable-by-design, bounded by the short TTL + key rotation). New neutral package `core/auth/servicetoken` (imports only `core/policysign` + stdlib — proven no import cycle) owns the mint primitive + the reserved-identity allowlist `{cordum-scheduler, api-gateway, workflow-engine}` as the single source of truth. Explicitly NOT a `SenderId/RequestedBy==defaultSenderID` exemption (that field is worker-forgeable). Attached at the scheduler's three self-reject emitters, the gateway cancel-ack result + cancel broadcast (cancel-ack `JobResult.WorkerId` is now set to `api-gateway` so the binding accepts it), and the workflow-engine cancel.
  - **Forge vuln (subject-binding) — found in review, not by CodeRabbit.** `verifySessionToken` switched on the verdict but DISCARDED `result.Claims`, so a provisioned worker A holding its OWN valid token could publish `JobResult{WorkerId:"w-victim"}` / `JobCancel{RequestedBy:"w-victim"}` and drive a victim job to SUCCEEDED/cancelled. The gate now binds `strings.TrimSpace(Claims.Subject) == packet identity` AND `safeSenderID(packet) == packet identity` on every token-verified packet (the warn/off/token-less paths, where Claims is nil, are intentionally untouched) and rejects + logs a SECURITY event on mismatch (never logging the token).
  - **Fail-open foot-gun (strict boot).** A typo'd `CORDUM_SDK_HANDSHAKE` (e.g. `enforse`) silently degraded to warn/admit. `buildSessionTokenMiddleware` now uses `ParseHandshakeModeStrict`, which refuses to boot on any non-empty value that does not round-trip to `off`/`warn`/`enforce`. The lenient `ParseHandshakeMode` is retained for non-boot callers.
- Non-spoofable rests on TWO jointly-required guards (omitting either reopens the hole): (1) the signed `typ` header is asserted on BOTH verify paths (a worker's `cordum-session` token cannot ride the no-active-check service path, and a service token cannot be presented as a worker token), and (2) the reserved control-plane identities are reserved at handshake-issue time, so a worker can never handshake AS a service and obtain a real worker token whose Subject impersonates a control-plane service.
- **Behavior change:** a typo'd `CORDUM_SDK_HANDSHAKE` now refuses to boot (previously degraded silently to warn). **New requirement:** the workflow-engine now needs the shared Ed25519 signing key (it loaded none before); enabling `enforce` fleet-wide requires the key deployed to scheduler + api-gateway + workflow-engine first, so `enforce` stays OFF until then. Cross-service admit/reject matrix verified by `TestEnforce_CrossServiceBroadcastMatrix`. General-purpose platform security hardening (demo-neutral).
...
#### cmd/cordum-scheduler — /health handler deps are now race-free during startup (2026-06-02, task-db950d76)

- The scheduler metrics/health HTTP server is started in a goroutine (`ListenAndServe`) with `healthDep` already registered as the `/health` handler, but its backing pointer fields (`jobStore`/`bus`/`safetyClient`) were assigned later on the main goroutine after multi-second blocking dependency init (Redis/NATS/safety-kernel construction). `healthDeps.ServeHTTP` reads exactly those three fields, and the struct had no synchronization — so a `/health` request landing in the startup window was an unsynchronized concurrent read/write (a genuine data race under the Go memory model and a `go test -race` failure, the canonical CI gate). Functional impact was benign (nil-checked fields read as `degraded`/503, the correct mid-startup answer); the real cost was UB / torn-pointer read + a flaky `-race` CI signal. **Fix:** `healthDeps` now carries a `sync.RWMutex`; a new `setDeps` publishes the three fields under the write lock and `ServeHTTP` snapshots them under the read lock before doing its (slower) Ping I/O outside the lock. The mutex (rather than reordering the server start to after dep wiring) is deliberate so `/health` keeps serving a correct `degraded`/503 liveness answer throughout dep init instead of refusing connections — avoiding a liveness probe killing the pod during slow startup. Readiness probes (which close over already-assigned locals) were never affected. A focused reproduction (`health_race_test.go`: 8 concurrent `/health` readers vs. a `setDeps` writer) is clean (`-race` verified in Linux/WSL CI per `CLAUDE.md`; this box is CGO-off, so `-count=3` is the local substitute). Confined to `cmd/cordum-scheduler/main.go` (+ `health_race_test.go`). General-purpose platform fix (demo-neutral).

#### cmd/cordum-scheduler + core/controlplane/scheduler — restored the dropped Phase-2 session-token wiring (2026-06-02, task-5c18f890 re-land via task-db950d76)

- task-5c18f890's worker session-token trust gate was QA-approved, but its implementation was partially lost during a shared-branch land-deliverables cascade (impl stranded in a stash while its tests were committed), leaving the `cmd/cordum-scheduler` and `core/controlplane/scheduler` test packages failing to compile (`undefined: buildSessionTokenMiddleware`; `*Engine has no method WithSessionMiddleware`) — a CI `go test ./...` build break that also blocked task-db950d76. **Fix:** restored verbatim from the approved change — `cmd/cordum-scheduler/main.go` `buildSessionTokenMiddleware`/`newSessionTokenMiddleware` + the `CORDUM_SDK_HANDSHAKE` wiring (back-compat admit when unset/off; fail-closed under `enforce` without key material; warn degrades to admit); `Engine.WithSessionMiddleware`; and the typed-first `auth_token` reads in `authTokenFromPacket`/`extractSessionToken` (cap/v2 `BusPacket.auth_token` field 18, with the legacy unknown-field fallback retained). The full approved suite (`session_middleware_test.go`, `engine_session_wiring_test.go`, `engine_session_verify_test.go`, `token_middleware_test.go`) now compiles and passes. The incomplete landing is tracked for re-QA in a follow-up ticket. General-purpose platform security fix (demo-neutral).

#### core/controlplane/safetykernel — decision cache no longer serves a stale ALLOW after an agent risk-tier/classification escalation (2026-06-02, task-8e3812d4)

- The decision-cache key (`cacheKeyForRequest`) hashes only the `PolicyCheckRequest` proto (minus `JobId`) plus the policy snapshot, computed BEFORE `enrichAgentContext` resolves the agent's `RiskTier`/`DataClassifications`/`Name`/`Team` from the (mutable) `AgentIdentityStore` into the local `PolicyInput` — and `matchRule` gates on those fields. Because `getCachedDecision` invalidates only on `policyVersion` + expiry, escalating an agent in the store (with no change to request bytes or policy) left a previously-cached ALLOW served for up to the cache TTL — a policy that should now DENY/REQUIRE_HUMAN kept serving the stale ALLOW; the 30s agent cache compounded the window. Severity LOW because the decision cache is off by default (`SAFETY_DECISION_CACHE_TTL` unset = 0); it only bites deployments that enable decision caching for horizontal scaling. **Fix:** requests carrying an `agent_id` label now bypass the decision cache (read AND write) when an agent store is wired — mirroring the existing velocity and action-descriptor bypasses — so an agent request always re-evaluates against the current identity and no cached ALLOW can outlive an escalation. The bypass is gated on `s.agentStore != nil`, so the label stays inert (and caching is preserved) for deployments without agent enrichment; non-agent requests and the cache-off default path are unchanged, and there is no extra Redis round-trip (the chosen bypass avoids the alternative of folding a store version/etag into the key). Confined to `core/controlplane/safetykernel/kernel.go` (+ tests). General-purpose platform security fix (demo-neutral).

#### dashboard — usePolicyAccess now fails CLOSED while auth-config is undefined (loading / fetch error) (2026-06-02, task-9cb66eb5)

- `usePolicyAccess` (`dashboard/src/hooks/usePolicyAccess.ts`) derived `requiresAuth = Boolean(!!authConfig && (...))`, so while `authConfig` was undefined — every initial render before `/auth/config` resolves, and persistently if that request 403s/errors/times out (`useAuthConfig` is a plain `useQuery` with no `initialData`) — `requiresAuth` collapsed to `false` and `derivePolicyAccess` returned EVERY capability `true` (`canEdit`, `canPublish`, `canRelease`, `canManageOutputRules`, `canManageTenants`; `isReadOnly:false`). Privileged affordances (Edit, Publish, Rollback, Release/Deny from quarantine, Manage tenants) rendered clickable for unauthenticated/viewer users on every policy-governance page load and permanently on auth-config fetch failure — the exact opposite of the sibling `usePermission`, which deliberately treats `authConfig === undefined` as fail-CLOSED (and has a passing test for it). **Fix:** `usePolicyAccess` now returns a fail-closed result (all caps `false`, unconditionally — so an admin mid-load is denied too) while `authConfig` is undefined, mirroring `usePermission`; the no-auth allow-all branch only triggers once the config has RESOLVED with every auth mode confirmed disabled (genuine no-auth deployments still work). The first render now denies, so there is no flash-of-allowed (deny → maybe-allow, not allow → retract). The backend Safety Kernel remains the real enforcement boundary; this corrects the client-side display contract. The parallel `ProtectedRoute` fail-open (mounts protected pages on auth-config error/undefined) is tracked as a follow-up in task-d59f3df2. Confined to `dashboard/src/hooks/usePolicyAccess.ts` (+ test); `src/api/generated` untouched. General-purpose platform (dashboard) security fix (demo-neutral).

#### dashboard — ProtectedRoute now fails CLOSED while auth-config is undefined (loading placeholder / error redirect) (2026-06-02, task-d59f3df2)

- `ProtectedRoute` (`dashboard/src/components/ProtectedRoute.tsx`) derived `requiresAuth = !!authConfig && (password_enabled||user_auth_enabled||saml_enabled||oidc_enabled)`, so while `authConfig` was undefined — every initial render before `/auth/config` resolves, and persistently if that request 403s/errors/times out (`useAuthConfig` is a plain `useQuery` with no `initialData`) — `requiresAuth` collapsed to `false`, `isAuthorized` became `true`, and the protected shell (`AppShell` + governance pages) MOUNTED for an unauthenticated user; the redirect-to-login effect (gated `!authLoading && !isAuthorized`) never fired because `isAuthorized` was already `true`. Same root cause as the sibling `usePolicyAccess` fail-open (task-9cb66eb5), one layer up (page MOUNT vs. affordance display). **Fix:** the gate is now three-state — while `authLoading` it renders a `role="status"` loading placeholder (the existing `LoadingScreen`) instead of mounting children or redirecting; once loading is done a missing config (`!authConfig`, covering an undefined/null error response) keeps `requiresAuth=true` so an unauthenticated user is redirected to `/login?returnUrl=…`; a RESOLVED config with every auth mode disabled still mounts (genuine no-auth deployments unaffected). The derivation is stateless, so a transient `/auth/config` error that later refetches re-evaluates and mounts (no latched-denied), and `/login` is rendered outside the protected tree so the error-state redirect cannot loop. The live-app gate is the fail-closed `useRequireAuth`/`ProtectedRoutes`, and the backend Safety Kernel remains the real enforcement boundary, so this closes a display-only page-shell mount / info-disclosure on a (currently unwired) latent component. Confined to `dashboard/src/components/ProtectedRoute.tsx` (+ `oidc-auth-gate.test.ts`, 5 added cases); `src/api/generated` untouched. General-purpose platform (dashboard) security fix (demo-neutral).

#### core/mcp — in-process session-taint store is now bounded (TTL + max-entry cap) (2026-06-02, task-e8960f8d)

- `inProcessTaintStore` (`core/mcp/taint_store.go`) wrote `s.m[key] = taint` with no TTL, no size cap, and no eviction (clean reads never delete, by design), accumulating one permanent entry per distinct `(tenant, session)` that ever tripped the prompt-injection scanner — for the whole process lifetime. This store is used when Redis is absent / `CORDUM_MCP_TAINT_BACKEND=memory`, AND as the per-instance fallback `RedisTaintStore` writes into on ANY Redis error. Because MCP/edge session ids are largely client-supplied, an attacker repeatedly tripping the scanner across fabricated session ids could drive monotonic map growth (memory-exhaustion DoS); a long-running single-instance or Redis-degraded gateway leaked under organic load too. **Fix:** the store is now bounded two ways, mirroring its siblings (`filterCache`'s cap+eviction and `RedisTaintStore`'s TTL): a per-entry TTL (resolved from `CORDUM_MCP_TAINT_TTL` / `defaultTaintTTL` = 1h, so the memory backend matches the Redis lifetime) checked on `GetTaint` and swept opportunistically, plus a `maxInProcessTaints` = 10 000 cap with batched oldest-by-expiry eviction (expired entries dropped first; if still over cap, the oldest are dropped to a low watermark in one sweep, so eviction amortizes to ~O(1) per write rather than O(n)). The bound is **fail-safe**: a missing/expired taint is the clean-case default, so eviction can only ever lose a taint (degrading the defense-in-depth content layer for one call — which still gets its own primary action-gate authorization — never fabricating one); a tainted session stays tainted within the cap+TTL. The Redis-backed store's TTL behavior is unchanged; the bound also protects the Redis-error fallback path. Confined to `core/mcp/taint_store.go` (+ tests). General-purpose platform hardening (demo-neutral).

#### core/controlplane/gateway/policybundles — replay/evals/simulation bundle merge now matches the kernel (install-recency + rule-ID dedup) (2026-06-02, task-0ba2bc0d)

- The gateway policy-bundle merge behind policy REPLAY, the EVALS runner, simulation, edge tier-evidence, and the Global policy view (`BuildPolicyFromBundles` → `MergeSafetyPolicies`) diverged from the authoritative safety-kernel merge: it sorted bundles ALPHABETICALLY (never reading `installed_at`) and appended rules with NO duplicate-id detection, while the kernel (`loadFragments`/`mergePolicies`) orders by `installed_at` ascending (recency wins) and dedupes rule ids last-seen-wins. Because `SafetyPolicy.Evaluate` is first-match, two co-installed bundles defining the same rule id with different decisions made replay/evals return the alphabetically-first bundle's decision while production returned the most-recently-installed one — so the tooling operators use to validate policy before rollout could predict ALLOW where production DENYs (or vice-versa). `MergeSafetyPolicies` also dropped `InputRules` entirely on multi-bundle merges. **Fix:** `BuildPolicyFromBundles` now mirrors `loadFragments` with two passes — PASS 1 hashes `{key, content}` in alphabetical order (so the `cfg:<sha>` snapshot stays install-order-independent), PASS 2 merges fragments by `installed_at` ASC with a bundle-key alphabetical tiebreak (new `bundleInstalledAt` mirrors the kernel's `fragmentInstalledAt`); `MergeSafetyPolicies` now dedupes Rules, OutputRules **and** InputRules by id last-seen-wins via a shared generic `mergeRulesByID` helper, matching `mergePolicies`. Replay/evals/simulation/tier-evidence/global now produce the same first-match decision production enforces. Single-bundle / no-duplicate behavior and the snapshot hash are unchanged. Confined to `core/controlplane/gateway/policybundles/{bundles.go,merge.go}` (+ tests); also repointed stale doc comments in `core/controlplane/safetykernel/global_policy.go` that wrongly attributed `FromSafetyPolicy`'s merged policy/snapshot and invariant precedence to the gateway's `BuildPolicyFromBundles`/`MergeSafetyPolicies` (the kernel actually uses `loadFragments`/`mergePolicies` and `applyKernelInvariants`; the safetykernel package makes zero code calls to the gateway funcs). The durable DRY follow-up — extracting ONE shared merge used by both the kernel and the gateway — is tracked in task-48567e75. General-purpose platform tooling-integrity fix (demo-neutral).

#### core/{infra/config,controlplane/safetykernel,controlplane/gateway/policybundles} — kernel adopts the gateway's any-enable OutputPolicy cross-fragment merge; per-caller merge option collapsed (2026-06-02, task-198acafd)

- Follow-up to task-48567e75 (which extracted ONE shared `config.MergePolicies` for the kernel loader + gateway bundle compiler but PRESERVED two residual divergences behind a `PolicyMergeOptions.MergeOutputPolicy` flag). The kernel cross-fragment merge kept only the oldest (base) fragment's `OutputPolicy` and ignored later fragments, while the gateway OR-merged `OutputPolicy.Enabled` and took the first non-empty `FailMode` — so for multiple bundles disagreeing on output policy, replay/evals/simulation (gateway) predicted output scanning ENABLED while the kernel enforced the oldest fragment's setting (the same replay-vs-prod gap class task-0ba2bc0d fixed for rules). **Fix:** the any-enable + first-non-empty-`FailMode` semantics are now the SINGLE canonical behavior for both callers — the kernel adopts the gateway's rule, so a guard enabled by ANY bundle stays enabled (fail-safe, consistent with the default-deny/fail-closed kernel rail and with both sides' invariants overlay, which already any-enabled `OutputPolicy`). `PolicyMergeOptions` (and the `MergeOutputPolicy` flag) is removed; `MergePolicies(base, extra)` is now optionless and both `safetykernel.mergePolicies` and `policybundles.MergeSafetyPolicies` delegate to it identically, so kernel and gateway produce identical `OutputPolicy` for multi-bundle inputs. The second divergence (gateway `CloneSafetyPolicy` dropping `SafetyPolicy.RequireHuman`, read by the kernel at `kernel.go:1530`) was already resolved by task-48567e75's shared clone, which preserves it; a regression test pins it. Single-bundle / common-case behavior is unchanged (any-enable only affects multi-bundle disagreement, and only ever flips `Enabled` false→true — never weakening output scanning). The kernel-enforcement change (oldest-wins → any-enable) was governor-signed-off. Confined to `core/infra/config/policy_merge.go` + the two delegating callers (+ tests). General-purpose platform tooling-integrity fix (demo-neutral).

#### core/controlplane/safetykernel — rules referencing an unknown scanner are now observable, not silently inert (2026-06-02, task-0c508199)

- `scanWithScanners` (`core/controlplane/safetykernel/output_policy.go`) silently `continue`s past any rule scanner/detector name not in the loaded registry, and neither `compileInputRules` nor `compileOutputRules` validated the names — so a single typo (`pii_detector` for `pii`) or a custom scanner that failed to load from `output_scanners.yaml` turned an entire `deny`/`quarantine` rule into a silent no-op: `evaluate{Input,Output}Rule` saw no findings → no match → content passed, on a component whose contract is fail-closed/default-deny. **Fix:** `compileInputRules`/`compileOutputRules` now receive the scanner registry (`s.scanners`, wired from `setPolicyWithInvariants`) and call `warnUnknownScanners`, which surfaces every unresolved scanner/detector name as a loud `slog.Warn` (rule id + unknown names + the known-scanner set) plus a registered `cordum_safety_unknown_scanner_total` counter — so the misconfiguration is observable at policy-load/hot-reload time instead of silent. Names are alias-normalized before the lookup (`secret_leak`→`secret`, `code_injection`→`injection`) and validated against the registry's built-ins **and** YAML-loaded scanners. The fix is deliberately **observability-only and behavior-preserving**: it does **not** drop the unknown name from the rule (pruning a rule's only scanner to empty would flip `evaluate*Rule` into a metadata-only **match-all** over-block) and does **not** fail the policy load (one typo must not brick a default-deny kernel). Confined to `core/controlplane/safetykernel/{output_policy.go,input_policy.go,kernel.go}` (+ tests). General-purpose platform security fix (demo-neutral).

#### core/edge/agentd — inline-wait approval now consumes the single-use approval before ALLOW (2026-06-02, task-2c330c82)

- With inline approval wait (the `cordumctl edge claude` default — `launcher_metadata.go:101` hardcodes `CORDUM_AGENTD_INLINE_APPROVAL_WAIT=true`), a `REQUIRE_APPROVAL` evaluate was resolved by `approvalRequiredDecision` → `WaitForApproval` (`POST /api/v1/edge/approvals/{ref}/wait`), a **read-only poll** that never calls `ClaimApproval` / sets `ConsumedAt`. On `Status=approved`, agentd returned `DecisionAllow`, so the record stayed `Status=approved, ConsumedAt=nil` and the EDGE-011 consume-once CAS was side-stepped — the same human approval authorized the destructive action on the inline-wait turn **and** could be consumed once more on a later identical `action_hash` (EDGE-039), i.e. two authorized executions from one approval. **Fix:** inline-wait ALLOW is now gated on a single-use consume CAS performed by that turn — after the wait reports approved, agentd re-issues `/edge/evaluate` carrying the `approval_ref` (`WaitForApproval=false`) so the Gateway's existing `consumeEdgeEvaluateApproval` → `ClaimApproval` CAS runs; ALLOW is returned only when the Gateway confirms the consume, otherwise it fails closed (DENY). A passive "approved" poll no longer authorizes execution. Confined to `core/edge/agentd/{approval_waiter.go,evaluator.go}`; the non-inline evaluate-with-`approval_ref` consume path, the rejection/timeout/degraded DENY paths, and the SafeAllowCache (which never caches `REQUIRE_APPROVAL`/`approval_ref` responses) are unchanged. General-purpose platform hardening.

#### core/workflow — standalone workflow-engine reconciler⇄engine run-lock self-contention fixed (2026-06-02, task-1b101328)

- In the deployed `cordum-workflow-engine` (part of the default compose stack), the engine is wired `WithRunLocker(jobStore)` and the reconciler is handed the **same** `jobStore` (`runner.go:130`/`:152`); both lock `runLockKey(runID)`. The reconciler acquired that key (`RedisJobStore.TryAcquireLock` is a non-reentrant `SetNX`), then — still holding it — called `engine.HandleJobResult`/`StartRun`, which re-acquired the **same** key via the engine's `lockManager`, hit contention, and returned `nil` doing **no work** (no error logged; the bus ACKs). Net: every `JobResult` delivery and every stuck-run reconcile was a fail-silent self-no-op — standalone runs never advanced past a dispatched step and stuck runs were never recovered. Invisible elsewhere because the gateway in-process engine (`gateway.go:600`) is built **without** a run-locker. **Fix:** the reconciler and engine now share exactly **one** acquisition of `runLockKey(runID)`. Added `lockManager.acquireLocal` (per-run *local* mutex only) plus lock-held engine entrypoints `handleJobResultLockHeld`/`startRunLockHeld`; the reconciler — which already holds the distributed lock — calls those, so the engine still takes the local mutex (preserving intra-process mutual exclusion, including the no-locker gateway path) but no longer self-contends on the distributed `SetNX`. Engine failures on the reconciler path now **surface** instead of being silently ACK-and-dropped. The reconciler also re-reads the run after its per-step advance loop before stuck-step recovery, since `RedisStore.UpdateRun` is a full-object overwrite. Confined to `core/workflow/{engine.go,engine_state.go,reconciler.go}`; gateway / delay-poller / API paths and the `core/infra/store` lock primitives are unchanged. General-purpose platform hardening (demo-neutral).

#### core/controlplane/gateway — governance health-check now verifies the audit HMAC chain (2026-06-02, task-b119e1d4)

- `governanceHealthDeps.VerifyChain` built `audit.VerifyOptions` without the HMAC key and never inspected `result.HMACSeen`, so for an HMAC-signed chain the HMAC tags were not verified (`HMACVerified=0`) yet `Status` stayed `ok`, mapping to `governance.ChainStatusOK` — a false-green on the `/api/v1/governance/health` chain-integrity factor (same false-green class as the compliance/SOC2-export fix, task-33485ac3). The health check now injects the server's HMAC key when HMAC is enabled (`s.auditChainer.HMACEnabled()` → `HMACKeyForVerify()`, sourced only from the configured chainer, never request input) and fail-closes `ok → partial` (→ `governance.ChainStatusPartial`) when HMAC-tagged events are seen but no key is configured — mirroring `/api/v1/audit/verify` (`handlers_audit_verify.go`). Confined to `core/controlplane/gateway/handlers_governance_health.go`; non-HMAC/dev chains still report `ChainStatusOK` (no over-downgrade). Regression tests in `handlers_governance_health_verifychain_test.go` cover no-key→partial, key→ok, wrong-key→compromised, and non-HMAC→ok. Sibling of task-33485ac3 (found by its DoD#2 sweep).

#### sdk/runtime — heartbeat & handshake BusPacket `trace_id` fix (2026-05-31, task-6e9c230d)

- The SDK worker-runtime heartbeat and handshake publishers emitted `BusPacket`s with an empty `trace_id`. The live path (`runtime.HeartbeatPayload`/`WithMemory`/`WithProgress`, used by every CAP worker) delegates to CAP's `capworker.HeartbeatPayloadWithProgress`, which hardcodes `TraceId:""`; the cordum wrapper's old `applyHeartbeatOptions` left it untouched (and returned the raw payload unchanged on the no-option path). CAP v2.x `ValidateBusPacket` rejects an empty `trace_id`, so the scheduler/gateway bus consumer dropped **every** `sys.heartbeat` (`bus: packet invalid … missing required field(s): trace_id`) → degraded/empty worker-capacity TTL registry and broken pool scheduling. The SDK now stamps `trace_id = workerID` (the natural correlation id, equal to `SenderId`): `sdk/runtime/bus_helpers.go` replaces `applyHeartbeatOptions` with `finalizeHeartbeatPayload`, which always re-stamps `trace_id` (and `created_at` when nil, to satisfy CAP's stricter SDK validator) before applying options; the inline `Worker` heartbeat/handshake builders in `sdk/runtime/worker.go` set `TraceId` in the struct literal *before* signing so it is inside the signed bytes. Two demo workers that build heartbeats inline (`demo/quickstart/worker`, `demo/mock-bank/worker`) were swept for the same omission. `sys.heartbeat` is a non-durable core-NATS subject (no JetStream backlog), so rejects stop the moment each emitter runs the fixed SDK. Regression tests `TestHeartbeatPayload_SetsTraceID`, `TestWorker_HeartbeatPacket_SetsTraceID`, `TestWorker_HandshakePacket_SetsTraceID` assert the published packets pass CAP `ValidateBusPacket`. The upstream CAP root cause (`HeartbeatPayloadWithProgress` hardcoding `TraceId:""`) is tracked separately in task-672ad19a. Sibling fix: the `sys.audit.export` `trace_id` fix in task-15f8c51f.

#### core/mcp — content session-taint scan-coverage hardening (2026-05-31, task-a3871d1b)

- The read-result prompt-injection scan (`scanResultForSessionTaint`) previously concatenated all `Content[].Text` plus `StructuredContent` and truncated at one global 256 KiB cap, so an injection in a field *after* earlier large content — or in `StructuredContent` once `Content` filled the cap, or split across the cap boundary — was never scanned and the session was never tainted, letting a subsequent destructive call through. Replaced `assembleResultScanContent` with `scanResultUnits`: each `Content[].Text` and each `StructuredContent` string leaf is scanned as its own bounded window, oversized fields are split with overlap (so a phrase straddling a boundary is still caught), and scanning stops at the first finding or a total-bytes/window-count DoS budget. Enforcement stays content/shape-aware (no tool-name glob); the clean-session negative control (no taint → delete allowed) is preserved. New `taint_policy_test.go` cases cover boundary, structured-content, overlap, negative-control, and DoS-bounded coverage. The deliberate fail-open taint posture is unchanged; a destructive-path fail-closed recommendation is captured separately (`Monday-demo/artifacts/taint-fail-open-decision.md`) pending governor sign-off.

#### core/audit — SIEM audit export `trace_id` fix (2026-05-30, task-15f8c51f)

- `NATSAuditPublisher.Send` now stamps a non-empty `TraceId` on the `sys.audit.export` BusPacket (correlation-first: `Extra["trace_id"]` → `EventHash` → generated UUID). CAP `ValidateBusPacket` rejects an empty/whitespace `trace_id`, so before this fix the bus NAK-looped **every** audit-export event and the `NATSAuditConsumer` — and therefore all four SIEM sinks (webhook/syslog/Datadog/CloudWatch) — was never reached. Confined to `core/audit/nats.go`; regression test `TestNATSAuditPublisher_SetsTraceID` asserts the precedence and that the published packet passes CAP validation.
