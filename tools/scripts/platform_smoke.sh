...
audit_resp=$(curl -fsS "${CURL_TLS_OPTS[@]}" "${auth_header[@]}" \
  "${API_BASE}/api/v1/audit/verify?tenant=${TENANT_ID}")
echo "${audit_resp}" | jq -e '.status == "ok" and .total_events > 0 and (.gaps | length) == 0' >/dev/null || {
