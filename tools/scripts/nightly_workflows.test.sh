...
def context_bad(*nodes): return any(isinstance(node, dict) and ((isinstance(node.get("env"), dict) and bool({"BASH_ENV", "ENV"} & set(node["env"]))) or bool(node.get("shell")) or bool((((node.get("defaults") or {}).get("run") or {}).get("shell")))) for node in nodes)
def check_job(path, job_id, steps, wanted, parent_context=()):
    ...
    for index, step in enumerate(steps):
        ...
        refs, calls = names_in(run), helper_calls(run); output_aliases = set(re.findall(r'(?m)^\s*(?:export\s+)?([A-Za-z_][A-Za-z0-9_]*)\s*=\s*["\']?\$\{?' + CREDENTIAL_NAME + r'\b', run))
        fallback_names = {name for name, value in env.items() if PLACEHOLDER in str(value)} if isinstance(env, dict) else set(); redacted = redact_names(run); issues.append("UNSAFE_SHELL_CONTEXT") if (calls or redacted or "quickstart_env_sharing_test.sh" in run) and context_bad(*parent_context, step) else None
        if fallback_names:
...
def check_quickstart_text(text):
    ...
    return [] if protected in text and leak_guard in text and all(0 <= generated < mask < dotenv < leak < log_print for generated, mask, leak in triples) else ["QUICKSTART_MASK_OR_LOG_ORDER"]
...
def scan(root):
    ...
    for path in paths:
        try: workflow = yaml.safe_load(path.read_text(encoding="utf-8")) or {}; jobs = workflow.get("jobs") or {}
        except Exception: issues.append((path.name, "-", "YAML_PARSE")); continue
        for job_id, job in jobs.items():
            key = (path.name, job_id); wanted = EXPECTED.get(key, [])
            found, credential_job = check_job(path.name, job_id, (job or {}).get("steps") or [], wanted, (workflow, job or {}))
...
def self_test():
    ...
    with TemporaryDirectory() as tmp:
        ...
        quick_red = all(check_quickstart_text(item) for item in (quick.replace('gha_mask_value "${seeded_redis}"', ""), quick.replace('grep -Fq "${seeded_redis}"', ""), quick.replace("seeded_redis=x\n", 'seeded_redis=x\ngrep -Fq "${seeded_redis}"\n'), quick.replace('gha_mask_value "${seeded_key}"', 'gha_mask_value "${seeded_key}" >/dev/null').replace('gha_mask_value "${seeded_redis}"', 'gha_mask_value "${seeded_redis}" >/dev/null'), quick.replace('source "${REPO_ROOT}/tools/scripts/github_actions_mask_env.sh"\n', 'source "${REPO_ROOT}/tools/scripts/github_actions_mask_env.sh"\ngha_mask_value(){ :; }\n'), quick.replace('"${quickstart_log}" || grep', '/dev/null || grep').replace('"${quickstart_log}"; then', '/dev/null; then'), quick.replace('; then\nfail ', ' || true; then\nfail '), *(quick.replace(command, f"# {command}") for command in ('gha_mask_value "${seeded_key}"', 'gha_mask_value "${seeded_redis}"', 'grep -Fq "${seeded_key}"', 'grep -Fq "${seeded_redis}"'))))
        upstream_red = all("UNSAFE_SHELL_CONTEXT" in check_job(path.name, "safe", parsed(safe), ["CORDUM_API_KEY"], context)[0] for context in (({"env": {"ENV": "/tmp/synthetic-shell-context"}},), ({"defaults": {"run": {"shell": "custom {0}"}}},))); quick_context_red = "UNSAFE_SHELL_CONTEXT" in check_job(path.name, "quick", [{"env": {"BASH_ENV": "/tmp/synthetic-shell-context"}, "run": "bash tools/scripts/quickstart_env_sharing_test.sh"}], [])[0]; return 0 if not check_quickstart_text(quick) and quick_red and upstream_red and quick_context_red and all(code in check_job(path.name, "safe", parsed(item), ["CORDUM_API_KEY"])[0] for item, code in mutations) else 1
